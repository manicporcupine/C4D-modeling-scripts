...
def _active_bd():
    ...
    return d.GetActiveBaseDraw() if d else None  # :contentReference[oaicite:1]{index=1}
...
def _transp_to_api(v):
    f = _safe_float(v)
    return f/100.0 if f > 1.0 else f  # проценты → 0..1

def _transp_to_txt(f):
    return f"{max(0.0, min(1.0, float(f)))*100.0:g}"  # 0..1 → проценты для UI
...
def _grab_current_dict():
    ...
    # читаем/пишем напрямую как рекомендует SDK (поля dbasedraw) :contentReference[oaicite:2]{index=2}
    return {
...
# -------- Tree model (одна колонка; без нумерации и без "???") --------
class PresetModel(gui.TreeViewFunctions):  # :contentReference[oaicite:3]{index=3}
    def __init__(self):
        self.items = []
        self.sel   = set()
...
class PresetDialog(gui.GeDialog):
    ...
    def InitValues(self):
        # сначала загружаем элементы (чтобы TreeView не рисовал пустую строку)
        self.model.items = _read_json()

        # затем настраиваем и привязываем TreeView
        layout = c4d.BaseContainer()
        layout.SetInt32(1, c4d.LV_TREE)
        self.tree.SetLayout(1, layout)
        self.tree.SetHeaderText(1, "Presets")
        self.tree.SetRoot(None, self.model, None)  # TreeViewFunctions → SetRoot :contentReference[oaicite:4]{index=4}
        self.tree.Refresh()
        return True
